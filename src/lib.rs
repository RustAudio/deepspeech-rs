...
impl Model {
    ...
    pub fn speech_to_text_with_metadata(&mut self, buffer: &[i16]) -> Result<Metadata, ()> {
        let ptr = unsafe {
            ds::DS_SpeechToTextWithMetadata(self.model, buffer.as_ptr(), buffer.len() as _)
        };
...
impl Metadata {
    ...
    pub fn num_items(&self) -> i32 {
        unsafe { (*self.metadata).num_items }
    }

    pub fn confidence(&self) -> f64 {
        unsafe { (*self.metadata).confidence }
    }
}
...
impl Stream {
    ...
    pub fn finish_with_metadata(self) -> Result<Metadata, ()> {
        let ptr = unsafe { ds::DS_FinishStreamWithMetadata(self.stream) };
        // Don't run the destructor for self,
        // as DS_FinishStream already does it for us
        forget(self);
        Ok(Metadata { metadata: ptr })
